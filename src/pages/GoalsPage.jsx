  ...
  const openEditModal = (goal) => {
    ...
    setFormData({
      ...
      milestones: Array.isArray(goal.milestones)
        ? goal.milestones.map(m => typeof m === 'string' ? m : m.title).join('\n')
        : ''
    });
    setIsModalOpen(true);
  };
      ...
      {filteredGoals.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.5rem' }}>
          {filteredGoals.map(goal => {
            ...
            const completedCount = milestones.filter(m => typeof m === 'object' ? m.completed : false).length;
                  ...
                  <div style={{ marginTop: 'auto' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {milestones.map((m, idx) => {
                        const isDone = typeof m === 'object' ? !!m.completed : false;
