...
export const StorageService = {
  ...
  exportAllData() {
    return JSON.parse(this.exportAllJSON());
  },

  importAllData(data) {
    const jsonStr = typeof data === 'string' ? data : JSON.stringify(data);
    const res = this.importAllJSON(jsonStr);
...
export const AuthService = {
  getCurrentUser() {
    ...
    const defaultUser = {
      ...
      avatar: profile.avatar || "PD",
      role: "Student",
      isLoggedIn: true
    };
    LocalStorageEngine.setItem(STORAGE_KEYS.AUTH, JSON.stringify(defaultUser));
    return defaultUser;
  },
